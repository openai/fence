...
- `script/test-selected-profile-runtime`
  ...
  - Plans `github_hosted_workflow_bootstrap_v1` by omission, refuses any mismatched planning descriptor or user allowlist entry, and reports the schema-`4` logical policy hash plus base-ruleset hash separately from the active TTL-derived ruleset hash.
...
- `script/test-action-wrapper`
  ...
  - Schema-`2` resident-health and live-systemd validators may land before the matching bundle, but must remain inactive for the schema-`1` bundled agent until the atomic `0.2.0` bundle refresh.
