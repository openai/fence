...
- Consumers must pin the full 40-character `action_commit` value from `action-release.json`; neither `main` nor the version tag is the consumption reference. Release notes and the final workflow summary must show the ready-to-copy form `uses: GrantBirki/fence@<D> # pin@vX.Y.Z` with the actual release tag so Dependabot can update the same-line version comment. Releases through `v0.6.3` retain their historical tag semantics.
