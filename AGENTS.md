...
- `script/test-profile-candidate`
  ...
  - Launches standard block networking plus measured sudo/container lockdown on the disposable workflow host using only explicit `github_hosted_https_baseline_candidate_v1` and zero user allowances.
  - The candidate is intentionally an open diagnostic baseline rather than a final status-only profile: it permits outbound TCP `443` plus measured hosted-runner platform DNS and host-control channels, and reports that later workflow code can use them for egress.
