...
The compatibility-first diagnostic reached terminal success in three hosted jobs. The current follow-up removes `codeload.github.com` because v0 does not support post-ready action downloads; the remaining wildcard DNS classes are still test-only egress limitations and are not a selected or default profile.
...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards the remaining GitHub-related compatibility classes after removing `codeload.github.com` plus bounded TTL-derived CNAME descendants returned by approved response chains, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  ...
  - Reports that post-ready codeload traffic is unsupported and that wildcard DNS authorization, approved HTTPS destinations, their resolved IP-address realization, and the root-resident resolver path remain egress limitations; it cannot become a default protected profile without repeated hosted proof and a separate reduction review.
