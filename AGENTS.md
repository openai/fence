...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards exact `vstoken.actions.githubusercontent.com`, `pipelines.actions.githubusercontent.com`, `payload.pipelines.actions.githubusercontent.com`, and `results-receiver.actions.githubusercontent.com` root queries plus bounded TTL-derived CNAME descendants returned by approved response chains, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  - Reports derived CNAME authorizations even when delegated DNS operator names leave GitHub suffixes; arbitrary suffix-shaped lookalike queries remain refused locally.
  - Materializes owned HTTPS address rules only from bounded A/AAAA answers for those exact approved root hostnames or derived CNAME descendants, refreshes the exact roots every five seconds, retains materialized rules for at most bounded observed TTL plus a fixed thirty-second refresh overlap, and atomically verifies each structured replacement; it proves an arbitrary suffix-shaped lookalike query is refused locally.
...
- The non-required `platform DNS block candidate` workflow may invoke `script/test-dns-block-candidate` on disposable hosts to test exact documented-services TTL-bounded materialization with host lockdown and terminal job finalization; it remains separate from required `integration` and does not select a default profile.
