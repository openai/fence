...
- `script/test-protected-run` is intentionally restricted to disposable GitHub-hosted Linux integration jobs because it launches the production standard-block lifecycle and leaves host network, sudo, and container controls resident without restore.
...
- `script/test-protected-run`
  - Linux x64-only, GitHub-Actions-only production-shaped standard-block entrypoint; do not run it on developer machines or reusable runners.
  ...
  - Proves that omitted `platform_profile` selects `github_hosted_job_status_v1`, native host network policy applies and verifies, measured sudo/container paths are disabled, DNS mediation remains resident, and late verification reports no critical findings.
  - Leaves all controls resident until ephemeral runner teardown. It must not restore access, run after-ready artifact/cache/API operations, or claim support beyond the reviewed standard-block path.
...
- Keep unsupported `run` modes fail-closed. Production readiness may be emitted only by the reviewed standard-block trusted-launcher lifecycle after required controls verify.
- Do not add a public `action.yml` wrapper until production audit and degraded semantics are reviewed, the standard lifecycle evidence is green, and an attested alpha binary exists. The intended later wrapper lives in this repository, uses an immutable external reference, carries a reviewed Linux binary, and does not download an agent at runtime.
