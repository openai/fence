...
- `script/test-protected-run` is intentionally restricted to disposable GitHub-hosted Linux integration jobs because it launches a production block lifecycle and leaves host network and sudo controls resident without restore; standard mode also leaves container controls disabled, while degraded mode deliberately preserves them.
...
- `script/test-dns-block-candidate`
  - Linux x64-only, GitHub-Actions-only destructive selected-profile runtime-evidence entrypoint; do not run it on developer machines or reusable runners. The historical script name remains while the selected-profile boundary is proved.
  - Plans `github_hosted_job_status_v1` by omission, refuses any mismatched planning descriptor or user allowance, and reports the schema-`3` logical policy hash plus base-ruleset hash separately from the active TTL-derived ruleset hash.
...
- `script/test-protected-run`
  - Linux x64-only, GitHub-Actions-only production-shaped block entrypoint; do not run it on developer machines or reusable runners.
  ...
  - Proves that omitted `platform_profile` selects `github_hosted_job_status_v1`, native host network policy applies and verifies, measured passwordless sudo is disabled, DNS mediation remains resident, and late verification reports no critical findings.
  ...
  - Leaves applied controls resident until ephemeral runner teardown. It must not restore access, run after-ready artifact/cache/API operations, or claim support beyond the reviewed block paths.
...
- The `platform selected profile runtime evidence` workflow may continue invoking three disposable `script/test-dns-block-candidate` replicas for broader measurement after one selected-profile runtime scenario joins required `integration`; neither path activates public protection or writes production readiness.
...
- Keep unsupported `run` modes fail-closed. Production readiness may be emitted only by reviewed trusted-launcher block lifecycles after their required controls verify.
- Do not add a public `action.yml` wrapper until production audit is reviewed, the standard and degraded lifecycle evidence is green, and an attested alpha binary exists. The intended later wrapper lives in this repository, uses an immutable external reference, carries a reviewed Linux binary, and does not download an agent at runtime.
