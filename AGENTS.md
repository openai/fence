...
- `script/classify-action-bundle-host`
  ...
  - Runs the bundle when the complete fingerprint matches. The checked-in bundle temporarily predates one exact source-accepted `90-cloud-init-users` digest, so Action acceptance may skip old-bundle activation only for that exact entry in `.github/action-bundle-host-transitions.json` after every other schema-`2` fact matches. Source-built integration must activate normally on the accepted host shape, and the refreshed bundle must remove the transition and restore normal bundled activation.
