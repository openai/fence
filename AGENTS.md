...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards only names matching the fixed GitHub compatibility hypothesis, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  - Pre-materializes fixed job-status names and updates owned HTTPS address rules only from bounded approved-name A/AAAA answers, with TTL expiry and atomic structured verification; it never authorizes an arbitrary DNS name or direct runner-user DNS egress.
  - Treats permitted wildcard DNS query labels as an explicit egress limitation; this candidate cannot become a default protected profile without a separately reviewed restriction of that channel.
...
- The non-required `platform DNS block candidate` workflow may invoke `script/test-dns-block-candidate` on disposable hosts to test TTL-bounded approved-name materialization with host lockdown and terminal job finalization; it remains separate from required `integration` and does not select a default profile.
