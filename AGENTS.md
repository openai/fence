...
The compatibility-first diagnostic reached terminal success in three hosted jobs. Removing `codeload.github.com` because v0 does not support post-ready action downloads also reached terminal success in three hosted jobs. The current follow-up removes the results-storage wildcard because v0 does not support post-ready artifact or cache storage traffic; the remaining wildcard DNS class is still a test-only egress limitation and is not a selected or default profile.
...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards the remaining GitHub-related compatibility classes after removing `codeload.github.com` and the results-storage wildcard plus bounded TTL-derived CNAME descendants returned by approved response chains, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  ...
  - Reports that post-ready codeload and results-storage traffic are unsupported and that wildcard DNS authorization, approved HTTPS destinations, their resolved IP-address realization, and the root-resident resolver path remain egress limitations; it cannot become a default protected profile without repeated hosted proof and a separate reduction review.
