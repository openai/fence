...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards the four GitHub-related compatibility classes modeled by the audit experiment plus bounded TTL-derived CNAME descendants returned by approved response chains, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  ...
  - Reports that wildcard DNS authorization, approved HTTPS destinations, their resolved IP-address realization, and the root-resident resolver path remain egress limitations; it cannot become a default protected profile without repeated hosted proof and a separate reduction review.
