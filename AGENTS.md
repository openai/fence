...
The current binary remains the Phase 3 non-enforcing agent model while Phase 4A evidence is collected: `render-plan` emits a deterministic native `nftables` preview, `check-support` exposes an accepted but not runtime-checked hosted-runner fingerprint reference, and `run` fails closed without writing readiness or claiming protection. Hidden Linux code reaches native network apply/verify/rollback, NFLOG ingestion, measured sudo/container lockdown, non-blocking host audit measurement, composed test-only ordering, and a strict-none host block candidate only through privileged hosted evidence. Resident network evidence, including non-blocking host audit measurement, may emit explicitly test-only readiness below a root-created non-production runtime root; separate disposable-host lockdown scenarios emit no readiness. Composed evidence may emit explicitly non-protecting readiness only after network policy and measured host lockdown both verify: one path isolates block rules inside a namespace to prove ordering, while the host candidate applies them on an ephemeral runner solely to test minimal finalization with no implicit profile. Neither path activates public protection. The complete protected lifecycle and any public GitHub Action wrapper are later reviewed changes.
...
- `script/test-host-block-candidate` is intentionally restricted to disposable GitHub-hosted Linux evidence jobs because it applies strict host network blocking and lockdown without restore.
...
- `script/test-host-block-candidate`
  - Linux x64-only, GitHub-Actions-only strict-none finalization-candidate entrypoint; do not run it on developer machines or reusable runners.
  - Launches one root transient service on the ephemeral host with standard block mode, `platform_profile: "none"`, and no user allowances; the service applies host network policy, disables measured sudo/container paths, remains resident, and never restores after test-only readiness.
  - Emits `host_block_candidate_test_only` evidence and `host_block_candidate_ready_no_public_activation` below non-production `/run/fence-host-candidate-*`; a successful workflow establishes only the measured minimal finalization path, not general compatibility or public protection.
