...
- The consolidated `CI` workflow in `.github/workflows/acceptance.yml` should expose exactly two required jobs on fixed `ubuntu-24.04`: `quality` and `acceptance`. The protected source-check set is exactly `quality`, `acceptance`, `action acceptance`, and `integration`.
- The `quality` job should validate locks, prepare Rust through the checksum-gated repository path, install committed coverage tooling, bootstrap offline inputs, run `script/lint`, and run the complete all-features test suite once through `script/test --coverage`.
...
- The non-destructive CI workflow should cancel stale pull-request runs but retain every `main` push by using a pull-request-number-or-commit-SHA concurrency key.
