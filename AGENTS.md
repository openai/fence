...
- `script/test-profile-candidate`
  ...
  - Launches standard block networking plus measured sudo/container lockdown on the disposable workflow host using only explicit `github_hosted_control_plane_candidate_v1` and zero user allowances.
  - The candidate is intentionally broader than a final status-only profile: it permits six fixed GitHub web/API and Actions control-service HTTPS destinations and reports that later workflow code can use them for egress.
