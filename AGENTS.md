...
- `script/test-action-wrapper`
  ...
  - Post-job failure diagnostics use shared report limits and a closed code allowlist. Update that allowlist and its regression coverage when adding resident failure codes. Never emit raw messages, claim verified protection from rejected evidence, or replace the original failure if logging fails.
