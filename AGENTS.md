...
- `script/classify-action-bundle-host`
  ...
  - Recursively validates the schema-`4` live observation and compares every enforced field against the bundled binary's schema-`2` `check-support` fingerprint, including trusted executables, permission ancestors, effective access, complete sudo policy, and the exact local-control inventory.
  - Runs the bundle when the complete fingerprint matches and permits a future source-before-bundle skip only for an exact source-reviewed sudo-policy digest listed in `.github/action-bundle-host-transitions.json`. The published bundle has no active transition entry and must activate normally.
  - Unknown digests, malformed or incomplete evidence, executable, ancestor, effective-access, resolver, principal, group, unit, socket, workload, local-control, or any other enforced fingerprint drift fail the job. The source fingerprint unit test must prove every future transition digest is accepted by current source. Once a refreshed bundle accepts the digest, classification automatically returns to normal bundled activation.
...
- The non-required `action bundle drift canary` runs daily and on manual dispatch against fixed `ubuntu-24.04`. It must fail unless the complete schema-`2` classifier authorizes normal bundle activation, then execute and verify the zero-input standard-block lifecycle without allowing transition skips, cancellation-dependent cleanup, access restoration, or unbounded diagnostic output.
