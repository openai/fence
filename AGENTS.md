...
- `script/test-dns-block-candidate`
  ...
  - Reports that post-ready codeload and results-storage traffic are unsupported and that bounded Actions-suffix DNS authorization, DNS query timing and count, approved HTTPS destinations, their resolved IP-address realization, bounded CNAME delegation, and the root-resident resolver path remain egress limitations; required integration exercises the test-only mechanism, but it cannot become a default protected profile without a separate promotion review.
...
- The `platform DNS block candidate` workflow may continue invoking three disposable `script/test-dns-block-candidate` replicas for broader measurement after one bounded DNS-mediated block scenario joins required `integration`; neither path selects a default profile or activates public protection.
