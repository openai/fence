...
The dormant trusted-launcher foundation is intentionally narrower than public activation. Linux code now models production runtime intake only at `/run/fence/<invocation-id>/config.json`, requires pinned root-owned directories containing only a root-owned `0600` regular config file before activation, uses no-follow and close-on-exec config opens, and validates that production execution is the root `MainPID` of `fence-<invocation-id>.service`. Public `run` still returns `enforcement_not_implemented`; no production lifecycle may call these primitives until the complete DNS-mediated network, sudo, container, readiness, resident verification, and no-restore composition is wired and proved in a later reviewed change.
