...
- `script/test`
  ...
  - Rootless coverage intentionally excludes `src/nft_backend.rs` and `src/nflog.rs`; those Linux privileged kernel/socket boundaries are proved through deterministic unit tests plus `script/test-privileged` namespace behavior tests on `ubuntu-24.04`.
