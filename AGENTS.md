...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards only exact `vstoken.actions.githubusercontent.com`, `pipelines.actions.githubusercontent.com`, and `results-receiver.actions.githubusercontent.com` queries documented by GitHub's public runner checks, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  - Materializes owned HTTPS address rules only from bounded A/AAAA answers for those exact approved hostnames, with TTL expiry and atomic structured verification; it proves a suffix-shaped lookalike query is refused locally.
