...
- `script/test-selected-profile-runtime`
  ...
  - Plans `github_hosted_workflow_bootstrap_v3` by omission, refuses any mismatched planning descriptor or user allowlist entry, and reports the schema-`6` logical policy hash plus base-ruleset hash separately from the active TTL-derived ruleset hash.
...
- `script/test-protected-run`
  ...
  - Proves that omitted `platform_profile` selects `github_hosted_workflow_bootstrap_v3`, native host network policy applies and verifies, direct host DNS mediation remains resident, and late verification reports no critical findings.
