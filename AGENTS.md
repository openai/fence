...
- `script/test-selected-profile-runtime`
  ...
  - Plans `github_hosted_workflow_bootstrap_v4` by omission, refuses any mismatched planning descriptor or user allowlist entry, and reports the schema-`8` logical policy hash plus base-ruleset hash separately from the active TTL-derived ruleset hash.
  ...
  - Routes host DNS directly through the mediator with a reviewed read-only resolver mount, pins and revalidates the unique runner-owned `Runner.Worker`, permits the exact `productionresultssa19.blob.core.windows.net` compatibility root, and permits at most four other exact results-storage accounts requested by that process. Workflow, Docker, ambiguous, drifted, or spoofed requesters cannot authorize the dynamic class. Authorized results-storage addresses, bounded Actions/GitHub-app suffix DNS authorization, DNS query timing and count, approved HTTPS destinations, bounded CNAME delegation, and the root-resident resolver path remain egress limitations.
...
- `script/test-protected-run`
  ...
  - Proves that omitted `platform_profile` selects `github_hosted_workflow_bootstrap_v4`, native host network policy applies and verifies, direct host DNS mediation remains resident, and late verification reports no critical findings.
...
- `script/observe-hosted-runner`
  ...
  - Tries at most three times to obtain two identical private local-control snapshots fifty milliseconds apart and reports stability, collection bounds, incomplete ownership or reachability, root `dockerd`/`containerd` identities, runner-reachable root Unix stream/seqpacket listeners, and root TCP listeners reduced to address family, wildcard/loopback/other-local class, and port. Filesystem Unix reachability is tested after dropping to the full runner identity so ordinary mode bits and ACLs are both effective; abstract listeners owned by a resolved root process are treated as reachable. Unix names are emitted only as domain-separated SHA-256 values. Owner evidence is limited to UID, a sanitized executable basename, one exact source-reviewed canonical executable path or the `unreviewed_executable_path` classification, one exact reviewed unified-cgroup identity or the `unreviewed_cgroup` classification, and process count. An unavailable or unreviewed executable or cgroup identity makes ownership incomplete. Socket inode/UID, process PID/start-time/executable device/inode pins, and private executable-path and cgroup fingerprints participate only in private stability comparison; the public projection strips them and aggregates equivalent listeners and container processes with positive multiplicities.
  ...
  - Must not establish support, enable public enforcement, write readiness, or leave host state after its bounded synthetic probes and runner-temporary evidence. Schema-`4` observations remain evidence-only and are not an authorization mechanism. During the bundle transition, the Action classifier recursively validates the schema-`4` envelope and projects only reviewed legacy fields into the schema-`1` comparison. Valid schema-`4`-only values—including `/usr/bin/test` state, ancestor probe results, local-control contents, and `unavailable`, `unstable`, or `bounds_exceeded` local-control outcomes—remain evidence-only and do not affect schema-`1` compatibility. Malformed or internally inconsistent schema-`4` evidence, data that prevents a complete legacy projection such as truncated or oversized sudo policy enumeration, and legacy-field mismatches fail closed. Schema-`1` compatibility does not accept or enforce the local-control inventory; schema `2` does that only after the inventory is separately reviewed.
...
- `script/classify-action-bundle-host`
  - GitHub-Actions-only compatibility classifier for source changes that update the accepted hosted-runner fingerprint before the next attested Action bundle refresh.
  - Compares the bounded live observation against the bundled binary's serialized `check-support` fingerprint, runs the bundle when it matches, and permits a skip only for an exact source-reviewed sudo-policy digest listed in `.github/action-bundle-host-transitions.json`.
  - Unknown digests, malformed evidence, executable, resolver, principal, group, unit, socket, workload, or any other fixed-fingerprint drift fail the job. The source fingerprint unit test must prove every transition digest is accepted by current source. Once a refreshed bundle accepts the digest, classification automatically returns to normal bundled activation.
  ...
  - The wrapper and hosted acceptance assertions must match the contract of the binary identified by `action/bundle-manifest.json`; update the wrapper schema/profile constants only in the same PR that refreshes the attested bundle. They require stable profile-realization fields, bounded results-storage evidence, a live matching systemd PID, fresh verification evidence, and the exact five-worker resident set including process attribution.
