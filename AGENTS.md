...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards only exact `pipelines.actions.githubusercontent.com` and `results-receiver.actions.githubusercontent.com` queries, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
...
- The non-required `platform DNS block candidate` workflow may invoke `script/test-dns-block-candidate` on disposable hosts to test exact-status-name TTL-bounded materialization with host lockdown and terminal job finalization; it remains separate from required `integration` and does not select a default profile.
