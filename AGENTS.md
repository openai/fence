...
- `script/test-action-wrapper`
  ...
  - Before rendering either mode, revalidate the protected root-owned report and the same live resident service across at most four rereads in one monotonic 160-millisecond evidence-settlement window. Reject changed policy or process identity, decreasing or malformed counters, and stale evidence; preserve critical reports for emission before failing the job. Do not add DNS warming, a control socket, a writable handshake, or an extra resident worker.
