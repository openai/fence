...
- `script/classify-action-bundle-host`
  ...
  - Runs the bundle when the complete fingerprint matches and permits a future source-before-bundle skip only for an exact source-accepted sudo-policy digest listed in `.github/action-bundle-host-transitions.json`. The published bundle has no active transition entry and must activate normally.
