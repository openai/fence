...
- `script/test-dns-block-candidate`
  ...
  - Materializes owned HTTPS address rules only from bounded A/AAAA answers for those exact approved root hostnames or derived CNAME descendants, with TTL expiry and atomic structured verification; it proves an arbitrary suffix-shaped lookalike query is refused locally.
