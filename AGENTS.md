...
- `script/test-dns-block-candidate`
  ...
  - Routes host and Docker DNS through fixed local listeners, forwards the Actions wildcard, the exact GitHub app receiver compatibility name, and bounded TTL-derived CNAME descendants returned by approved response chains, and permits upstream UDP DNS only from the root-resident mediator through an owned nftables rule.
  ...
  - Reports that post-ready codeload and results-storage traffic are unsupported and that wildcard DNS authorization, approved HTTPS destinations, their resolved IP-address realization, bounded CNAME delegation, and the root-resident resolver path remain egress limitations; it cannot become a default protected profile without a separate constrained-design review.
...
- The non-required `platform DNS block candidate` workflow may invoke `script/test-dns-block-candidate` on disposable hosts to test GitHub-related compatibility-class TTL-bounded materialization with host lockdown and terminal job finalization; it remains separate from required `integration` and does not select a default profile.
