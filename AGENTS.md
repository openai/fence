...
The current binary remains the Phase 3 non-enforcing agent model while Phase 4B compatibility evidence is collected: `render-plan` emits a deterministic native `nftables` preview, accepts an explicitly requested non-default static `github_hosted_job_status_v1` candidate, `check-support` exposes an accepted but not runtime-checked hosted-runner fingerprint reference, and `run` fails closed without writing readiness or claiming protection. Hidden Linux code reaches native network apply/verify/rollback, NFLOG ingestion, measured sudo/container lockdown, non-blocking host audit measurement, composed test-only ordering, and host-block candidate proof only through privileged hosted evidence. Resident network evidence, including non-blocking host audit measurement, may emit explicitly test-only readiness below a root-created non-production runtime root; separate disposable-host lockdown scenarios emit no readiness. Composed evidence may emit explicitly non-protecting readiness only after namespace-isolated block networking and measured host lockdown both verify; because its blocking rules do not protect the host network, it is not containment proof. A separate non-required candidate workflow may enforce the fixed two-host status profile on the disposable host while retaining controls through teardown, but it does not select a default profile or activate public enforcement. The complete protected lifecycle and any public GitHub Action wrapper are later reviewed changes.
...
- `script/test-profile-candidate` is intentionally restricted to non-required disposable GitHub-hosted Linux evidence jobs because it applies host block policy and lockdown without restore while testing terminal completion.
...
- `script/test`
  ...
  - Rootless coverage intentionally excludes `src/composed.rs`, `src/nft_backend.rs`, `src/nflog.rs`, `src/runtime.rs`, `src/lifecycle.rs`, and `src/lockdown.rs`; those Linux privileged kernel/socket, no-follow runtime-file, transient-service, composed-ordering, and host-lockdown boundaries are proved through deterministic unit tests plus hosted privileged evidence on `ubuntu-24.04`.
...
- `script/test-profile-candidate`
  - Linux x64-only, GitHub-Actions-only Phase 4B candidate entrypoint; do not run it on developer machines or reusable runners.
  - Launches one root transient service on the disposable host using explicit `github_hosted_job_status_v1`, no user allowances, native host networking, and the standard measured sudo/container lockdown.
  - The profile contributes only fixed pre-resolved TCP port `443` allowances for `pipelines.actions.githubusercontent.com` and `results-receiver.actions.githubusercontent.com`; no wildcard, dynamic authorization, cache, artifact, repository API, action-download, or storage destination is implicitly permitted.
  - Emits only `host_block_candidate_test_only` and `host_block_candidate_ready_no_public_activation` evidence beneath non-production `/run/fence-profile-candidate-*`, leaves controls resident until ephemeral teardown, and must not write production readiness or claim protection.
...
- The `integration` workflow should run only on `ubuntu-24.04`, prepare the pinned Rust toolchain through repository scripts, invoke `script/observe-hosted-runner` for bounded read-only runner-shape evidence, invoke `script/test-privileged` for namespace-isolated `network_enforcement_test_only` and `resident_lifecycle_test_only` evidence, isolate `script/test-lockdown` audit, rollback, degraded, and standard host-lockdown scenarios on disposable runners, run `script/test-composed` for namespace-network plus host-lockdown ordering evidence, and run `script/measure-platform-egress` around controlled GitHub metadata/artifact operations in a separate non-blocking host-audit job while preserving the stable aggregate `integration` context.
- The non-required `platform profile candidate` workflow may run three disposable host-block trials with `script/test-profile-candidate`; it must remain outside the required aggregate until two identical-head executions reach six terminal successes and a later reviewed change selects the profile.
...
- `src/composed.rs`, `src/lifecycle.rs`, `src/lockdown.rs`, `src/nft_backend.rs`, `src/nflog.rs`, and `src/runtime.rs` are explicit privileged-boundary exceptions: their composed-ordering, host-lockdown, kernel-state, netlink-socket, no-follow root-file, and transient-service execution paths are validated by `integration`, while deterministic model, bounding, path-safety, scheduling, and prefix-to-metadata logic retain ordinary unit tests.
