...
This repository is the Rust implementation scaffold for Fence, a security agent intended to harden a narrowly supported CI runner class. The v0 protected target is GitHub-hosted Ubuntu LTS x64, selected through either `ubuntu-24.04` or `ubuntu-latest`, with a native Linux GNU agent artifact; see `docs/v0.md` for the normative behavior contract and `docs/threat-model.md` for security claims and residual risks. It is bootstrapped from a hermetic, reproducible, air-gapped-leaning Rust template. Treat it as public open source infrastructure: every file, comment, workflow, log line, and document change may be visible to anyone.
...
- `script/test-action-setup-failure` and `script/test-action-tamper` are intentionally restricted to disposable GitHub-hosted Linux Action-acceptance jobs. The former proves malformed native and raw wrapper input fails before mutation and that a valid source-side required-host prehydration failure is reported promptly through a retained failed transient unit without mutating controls. The latter launches the bundled audit lifecycle, deletes owned network state after readiness, proves resident critical drift, invokes the post hook expecting failure, and never restores access.
...
- `script/test-lockdown`
  ...
  - Executes exactly one `acl-reject`, `audit`, `local-control-reject`, `rollback`, `unsafe-preserve`, or `standard` scenario inside a root transient `systemd` service after verifying its security-relevant runner capabilities.
...
- `script/verify-protected-finalization`
  ...
  - In Action mode, separately validate exactly one successful, prefix-aware zero-input standard job and its exact activation, bounded blocked-burst evidence step, and post hook. Its single schema-`1` structured report must preserve verified strict-block controls, disclose bounded row omissions as a warning, and contain the two final exact blocked test-only IPv4 destinations on UDP port `443`. The three quiet replicas must remain separate, retain healthy verified controls, and meet the unchanged 180-second deadline. Their reports may contain fully classified noncritical DNS refusals, but no other warnings or omissions.
...
- `script/observe-hosted-runner`
  ...
  - Emits and retains under the runner-provided temporary root schema-`4` bounded JSON describing only the runner principal/group names; fixed command canonical targets, types, owner/group classifications, modes, device/inode identities, and runner writability; the same metadata for bounded sudo-policy sources; reviewed resolver symlink target/type/mode/owner; fixed container-unit state; optional bounded root-owned Azure platform-agent diagnostics; fixed runtime socket state; aggregate Docker workload count; complete sudo-policy source digests plus reduced principal/group `NOPASSWD` marker classification; and a bounded closed local-control candidate inventory.
  - Serializes only reviewed Ubuntu LTS identity, runner principal, runner groups, fixed canonical targets, resolver target, sudo source identities, systemd states, and container-socket owner/group names. Unknown runner groups and sudo source names use domain-separated SHA-256 identities where uniqueness matters; other unreviewed values use fixed classifications. Raw runner group names remain private to the collector only for effective identity and sudo-marker classification, and every serialized dynamic string is printable ASCII.
  ...
  - Must not establish support, enable public enforcement, write readiness, or leave host state after its bounded synthetic probes and runner-temporary evidence. Schema-`4` observations remain evidence-only and are not themselves an authorization mechanism. The Action classifier validates the bounded complete envelope and supported runner identity, then lets the Rust resident verify privileged host security before readiness. Malformed or internally inconsistent evidence, truncated or oversized sudo enumeration, and incomplete or unstable local-control acquisition fail closed.
...
- `script/observe-wireserver-agent`
  ...
  - Optionally observes natural connect attempts from reviewed root-owned Azure platform-agent processes, retains matches only for the exact Azure platform address and TCP ports `80` and `32526`, and bounds process count, transient trace bytes, duration, and results. Missing agent diagnostics never block activation or weaken root-only WireServer firewall enforcement.
...
- `script/test-action-wrapper`
  ...
  - Before rendering either mode, revalidate the protected root-owned report and the same live resident service across at most four rereads in one monotonic 160-millisecond evidence-settlement window. Reject changed policy or process identity, malformed counters, any decrease in resident-lifetime sampled counters, and stale evidence. Accept a lower firewall counter only when a newly validated block-mode ruleset hash identifies the next firewall epoch; audit and unchanged-epoch firewall counters must never decrease. Preserve critical reports for emission before failing the job. Do not add DNS warming, a control socket, a writable handshake, or an extra resident worker.
  ...
  - Native `allow_github_artifacts` accepts only lowercase `true` or `false`, remains disabled when omitted, rejects audit-mode activation and mixed raw/native configuration before mutation, and requires matching opt-in status, bounded exact-account provenance, and explicit warning evidence from the source agent.
  ...
  - DNS refusal warnings must report only the five fixed `materialization_rejection_reasons` counters: `invalid_response`, `authorization_changed`, `capacity`, `queue_unavailable`, and `firewall_update_failed`. Validate their bounded counts and total before rendering them. Do not describe every refused DNS answer as a firewall failure; actual firewall-update failures remain critical.
  - Keep successfully blocked results-storage requests visible in human-readable activity and `warnings.results_storage_rejections` without marking verified protection as a warning. Preserve warnings for attribution failures, exhausted account capacity, weakened controls, missing or truncated evidence, and critical drift. In `audit` mode, describe an exhausted limit as what block mode would deny, never as an actual denial.
  - On rejected post-job report validation, emit only the bounded unverified diagnostic projection: fixed labels, safe numeric health fields, and capped codes from the closed source-reviewed vocabulary. Never emit raw messages or reinterpret diagnostic evidence as verified protection; preserve the original failure even if logging fails.
...
- `script/classify-action-bundle-host`
  ...
  - Validates complete bounded schema-`4` observation evidence and reviewed Ubuntu LTS x64 runner identity, then delegates privileged security verification to the Rust resident.
  - Never skips Action activation for routine runner-image differences or source-known policy transitions; malformed, incomplete, or unsupported runner evidence fails closed.
  ...
  - The wrapper and hosted acceptance assertions must match the source agent contract; update wrapper schema/profile constants, tests, and the source version in the same reviewed PR. Release automation builds the matching bundle after merge. They require stable profile-realization fields, bounded results-storage evidence, a live matching systemd PID, fresh verification evidence, and exactly five resident workers with Docker or exactly three when Docker is absent; process attribution is always required.
...
- `script/test-action-setup-failure`
  ...
  - Invokes the dependency-free Action launcher with malformed invocation, wildcard, over-limit native allowlist, malformed artifact compatibility, audit-mode artifact compatibility, unsupported raw profile, duplicate raw JSON field, scoped IPv6, and restricted results-storage inputs; proves rejection occurs before Action state, runtime directories, transient services, or owned nftables state are created and before passwordless sudo is changed.
  - Passes one otherwise valid reserved `.invalid` hostname through the raw-config wrapper boundary, proves the bundled source agent reports `dns_block_prehydration_failed` before control mutation, and requires the launcher to retain the failed transient unit, fail before the readiness timeout, and emit only the structured Fence error code from the unit journal.
...
- Retain the assembled source-candidate artifact for 30 days so an older failed-job rerun can reuse its exact verified bundle. Keep the small artifact-compatibility fixture at one day; do not change release-artifact retention or provenance checks.
- The non-required `nightly` workflow should run daily, by input-free manual dispatch, and for same-repository pull requests targeting `main`. Scheduled and manual runs must use exact `main`; pull-request runs must use GitHub's exact same-repository merge commit. Build an ephemeral production-shaped candidate on fixed `ubuntu-24.04` and reuse the complete unique-case Action-acceptance suite on `ubuntu-latest`. It is not a branch-protection or release gate; only reviewed Ubuntu LTS versions and security-relevant capabilities are accepted.
- The non-required `action drift canary` must remain read-only, run every six hours, support manual dispatch with an optional full Action SHA, and expose reusable execution with a required full Action SHA. Every scheduled, manual, and reusable invocation must verify zero-input standard block and explicit audit mode on both `ubuntu-24.04` and `ubuntu-latest` without skipping activation or restoring access.
