...
- `script/observe-hosted-runner`
  ...
  - Must not establish support, enable public enforcement, write readiness, or leave host state after its bounded synthetic probes and runner-temporary evidence. Schema-`4` observations remain evidence-only and are not an authorization mechanism. During the bundle transition, the Action classifier recursively validates the schema-`4` envelope, requires stable, complete, within-bound local-control evidence and untruncated, non-oversized sudo policy input, and projects only legacy fields into the schema-`1` comparison. Malformed projection data, unstable, unavailable, or bounds-exceeded local-control evidence, truncated or oversized sudo enumeration, and legacy-field mismatches fail closed. Valid schema-`4`-only values—including an absent `/usr/bin/test`, an unavailable ancestor probe, and listener or container identity contents—remain evidence-only and do not affect the schema-`1` comparison.
