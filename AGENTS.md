...
- `script/test-profile-candidate`
  ...
  - Launches standard block networking plus measured sudo/container lockdown on the disposable workflow host using only explicit `github_hosted_https_only_candidate_v1` and zero user allowances.
  - The candidate is an intentionally open diagnostic reduction rather than a final status-only profile: it permits arbitrary outbound TCP `443`, removes the earlier explicit hosted-runner platform DNS/host-control allowances, and reports that later workflow code can still use HTTPS for egress.
