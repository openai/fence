...
fn static_job_status_profile_host_block_candidate_worker() {
    ...
    let config = format!(
        r#"{{"schema_version":1,"mode":"block","invocation_id":"{invocation_id}","platform_profile":"github_hosted_job_status_v1","container_policy":"disable","allowances":[]}}"#
    );
    let plan = build_plan(
