...
fn broad_control_plane_profile_host_block_candidate_worker() {
    ...
    let config = format!(
        r#"{{"schema_version":1,"mode":"block","invocation_id":"{invocation_id}","platform_profile":"github_hosted_control_plane_candidate_v1","container_policy":"disable","allowances":[]}}"#
    );
    let plan = build_plan(
