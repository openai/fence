...
use crate::plan::{
    AssuranceStatus, GITHUB_HOSTED_HTTPS_UDP_DNS_CANDIDATE_PROFILE_ID, PlanData,
    PlatformProfilePlan,
};
...
fn validate_host_block_candidate_plan(plan: &PlanData) -> Result<(), ComposedError> {
    validate_standard_block_plan(plan)?;
    if plan.platform_profile.id != GITHUB_HOSTED_HTTPS_UDP_DNS_CANDIDATE_PROFILE_ID
        || plan.platform_profile.selection_status != "explicit_open_https_udp_dns_not_default"
        || !plan.requested_policy.is_empty()
    {
        return Err(ComposedError::new(
            "invalid_host_block_candidate_policy",
            "host block candidate accepts only the explicit GitHub-hosted HTTPS-plus-UDP-DNS profile with no user allowances",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn candidate_plan(invocation: &str) -> PlanData {
        let json = format!(
            r#"{{"schema_version":1,"mode":"block","invocation_id":"{invocation}","platform_profile":"github_hosted_https_udp_dns_candidate_v1","container_policy":"disable","allowances":[]}}"#
        );
        build_plan(
            parse_and_normalize(json.as_bytes()).unwrap(),
            &LiteralResolver,
    ...
    #[test]
    fn labels_https_udp_dns_candidate_without_a_public_protection_assertion() {
        let operations = Rc::new(RefCell::new(Vec::new()));
        let (network, lockdown) = fakes(&operations);
        ...
        assert!(
            session
                .evidence
                .limitations
                .contains(&"candidate_is_intentionally_open_https_udp_dns_only")
        );
    ...
    #[test]
    fn accepts_only_explicit_https_udp_dns_for_host_block_candidate() {
        ...
        let json = r#"{"schema_version":1,"mode":"block","invocation_id":"candidate-user","platform_profile":"github_hosted_https_udp_dns_candidate_v1","container_policy":"disable","allowances":[{"destination_type":"ip","destination":"192.0.2.18","protocol":"tcp","port":443}]}"#;
