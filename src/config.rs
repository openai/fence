...
pub enum PlatformProfile {
    None,
    GithubHostedHttpsOnlyCandidateV1,
}
...
impl PlatformProfile {
    pub fn id(self) -> &'static str {
        match self {
            Self::None => "none",
            Self::GithubHostedHttpsOnlyCandidateV1 => "github_hosted_https_only_candidate_v1",
        }
    }
}
...
pub fn parse_and_normalize(bytes: &[u8]) -> Result<NormalizedConfig, ErrorDetail> {
    ...
    let platform_profile = match input.platform_profile.as_deref().unwrap_or("none") {
        ...
        "github_hosted_https_only_candidate_v1" => {
            PlatformProfile::GithubHostedHttpsOnlyCandidateV1
        }
        _ => {
            return Err(ErrorDetail::new(
                "invalid_platform_profile",
                "platform_profile must be none or github_hosted_https_only_candidate_v1",
            )
...
mod tests {
    ...
    fn accepts_audit_without_lockdown_degraded_block_and_explicit_candidate_profile() {
        let audit = parse_and_normalize(
            br#"{"schema_version":1,"mode":"audit","invocation_id":"audit-1","platform_profile":"github_hosted_https_only_candidate_v1","allowances":[]}"#,
        )
        .unwrap();
        let degraded = parse_and_normalize(
            br#"{"schema_version":1,"mode":"block","invocation_id":"block-1","platform_profile":"github_hosted_https_only_candidate_v1","container_policy":"unsafe_preserve","allowances":[]}"#,
        )
        .unwrap();
