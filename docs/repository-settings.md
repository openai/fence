...
- Keep the `platform profile candidate` workflow non-required while it tests explicit disposable-host egress candidates. Its current HTTPS-baseline candidate intentionally permits arbitrary outbound TCP `443` and measured hosted-runner platform DNS/host-control channels to establish terminal compatibility before minimization; it is not a public protection gate or a default profile. Keep its concurrency commit-scoped because a deliberately blocking failed candidate may be unable to receive cancellation and must not queue a corrected candidate indefinitely.
