...
- Keep the `integration` workflow required as the distinct hosted-runner observation and privileged evidence gate. It includes namespace-isolated native network/resident proof and separate disposable-runner lockdown scenarios; its public boundary remains evidence-only and it does not assert protection.
