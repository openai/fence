...
- Keep the `platform profile candidate` workflow non-required while it tests explicit disposable-host egress candidates. Its current broad compatibility candidate intentionally permits reported GitHub web/API, Actions control-service, results-storage, and measured hosted-runner platform DNS/host-control channels for compatibility evidence; it is not a public protection gate or a default profile. Keep its concurrency commit-scoped because a deliberately blocking failed candidate may be unable to receive cancellation and must not queue a corrected candidate indefinitely.
