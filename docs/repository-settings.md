...
- Keep the `platform profile candidate` workflow non-required during Phase 4B. It tests the explicit two-host `github_hosted_job_status_v1` candidate in three disposable host-block jobs per execution; do not promote that profile or change required contexts unless two identical-head executions complete all six candidate jobs successfully and a later reviewed change records the decision.
