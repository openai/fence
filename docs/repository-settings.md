...
- Keep the `platform profile candidate` workflow non-required while it tests explicit disposable-host egress candidates. Its current broad control-plane candidate intentionally permits reported GitHub web/API and Actions control-service channels for compatibility evidence; it is not a public protection gate or a default profile.
