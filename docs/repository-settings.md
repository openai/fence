...
- Keep the `action acceptance` workflow as the distinct bundled-wrapper gate: validate the committed release-bound bundle, invoke the root Action through `uses: ./`, prove standard block, degraded `unsafe_preserve`, bounded `*.docker.io` authorization, and audit behavior on disposable GitHub-hosted runners while controls remain resident, prove quiet terminal jobs retain downloadable logs, prove the registered post runtime, writable ancestor guards, and bundled binary resist runner-user pathname replacement, reject malformed setup and wildcard inputs before mutation, and prove post-ready critical drift fails the protected post hook without restore. During the active bounded source-before-bundle window, destructive checked-in bundle activation may be skipped only when every non-digest fingerprint fact matches and the exact source-accepted sudo-policy digest appears in `.github/action-bundle-host-transitions.json`; malformed, incomplete, unknown, or combined drift must fail. The refreshed bundle must clear the transition and restore normal activation.
