...
- Keep the `platform profile candidate` workflow non-required while it tests explicit disposable-host egress candidates. An open HTTPS-plus-platform-channel baseline reached terminal compatibility; its current reduction candidate permits arbitrary outbound TCP `443` while removing explicit platform DNS/host-control allowances. It is not a public protection gate or a default profile. Keep its concurrency commit-scoped because a deliberately blocking failed candidate may be unable to receive cancellation and must not queue a corrected candidate indefinitely.
