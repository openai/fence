...
- Keep the `action acceptance` workflow as the distinct bundled-wrapper gate: validate the committed release-bound bundle, invoke the root Action through `uses: ./`, prove standard block, degraded `unsafe_preserve`, and audit behavior on disposable GitHub-hosted runners while controls remain resident, reject malformed setup before mutation, and prove post-ready critical drift fails the post hook without restore.
...
- Keep the root `action.yml` wrapper in this repository and document external consumption only through immutable commit SHAs. It must carry a checksum-validated attested Linux release binary, bind that binary to a schema-`2` manifest with a matching immutable stable or prerelease release channel, default omitted input to strict standard block with an empty `allowlist`, launch the root transient service from fixed local inputs, render bounded local evidence, and avoid runtime agent downloads, policy fetches, stop operations, or access restoration.
