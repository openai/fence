...
- **Resident agent -> post hook:** root-owned `ready.json`, `report.json`, and
  `dns-report.json` are runner-readable but not runner-writable. The post hook
  also verifies the live systemd PID, evidence freshness, protected mount, and
  runtime digest before trusting the report (`src/runtime.rs`,
  `action/post.cts`).
...
6. **Forge or replay evidence:** later code writes a false report or leaves a
   stale healthy file after killing the service. Root ownership, active
   MainPID checks, worker health, monotonic verification sequence, and a
   20-second freshness bound reject the evidence.
...
| TM-004 | Malicious workflow process | Writable launcher/post/binary or evidence path | Replace validator or forge healthy evidence | False success after lost controls | Agent, post hook, reports | Root-owned copy, read-only bind mount, digests, no-follow files, live PID/freshness checks (`action/main.cts`, `action/post.cts`, `src/runtime.rs`) | Kernel or privileged mount bypass is out of scope | Keep exact runtime file manifest and hosted tamper tests | Post-hook integrity or freshness failure | Low | High | Medium |
