...
The source tree defines the schema-`4` policy and schema-`2` runtime-evidence
hardening contract. The committed Action bundle remains governed by
`action/bundle-manifest.json` and its wrapper schema constants. It does not gain
schema-`2` liveness and freshness guarantees until the matching attested agent
and wrapper validation constants are refreshed together. That atomic bundle is
the critical-adoption gate for this model.
...
- **Resident agent -> post hook:** root-owned `ready.json`, `report.json`, and
  ...
  checks apply at the wrapper boundary. The schema-`1` bundled agent does not
  claim the new PID/freshness guarantees (`src/runtime.rs`,
  `action/post.cts`).
...
6. **Forge or replay evidence:** later code writes a false report or leaves a
   stale healthy file after killing the service. In the schema-`2` adoption
...
| TM-004 | Malicious workflow process | Writable launcher/post/binary or evidence path | Replace validator or forge healthy evidence | False success after lost controls | Agent, post hook, reports | Root-owned copy, read-only bind mount, digests, no-follow files, and schema-`2` live PID/freshness checks (`action/main.cts`, `action/post.cts`, `src/runtime.rs`) | The schema-`1` bundle does not enforce the new liveness/freshness contract; kernel or privileged mount bypass is out of scope | Refresh the attested agent and wrapper schema constants atomically; keep exact runtime manifest and hosted tamper tests | Post-hook integrity or freshness failure | Medium until schema-`2` bundle, then Low | High | High until bundle gate, then Medium |
