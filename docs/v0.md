...
explicit test-only readiness proves setup ordering, not host protection. None
of these paths selects a platform profile. A strict-none host attempt
completed local checks but did not produce a completed workflow result before
cancellation. A follow-up candidate applies the composed block lifecycle on
one disposable host with exactly one explicit test allowance for GitHub's
documented log/summary receiver; successful job completion can inform the
profile decision, not activate public protection. This evidence is not
...
Phase 4A may additionally execute a dedicated host candidate on a separate
ephemeral runner. That candidate accepts only standard block mode,
`platform_profile: "none"`, and exactly one ordinary policy allowance for
GitHub's documented `results-receiver.actions.githubusercontent.com:443`
log/summary endpoint; it applies the composed controls on the host and may write only
`host_block_candidate_ready_no_public_activation` with
`host_block_candidate_test_only` evidence. Its job has no post-ready network
operation by design: successful workflow completion is evidence for one
minimal finalization candidate with controls left installed until teardown.
The allowance is encoded as test policy because a public built-in profile is
not selected; it is available to later code and is therefore an explicit
egress limitation. This is not a public protection assertion, an Action
interface, or proof that other workflows remain compatible.
...
protection available to public execution. A distinct documented-log host
candidate may emit `host_block_candidate_test_only` and
`host_block_candidate_ready_no_public_activation` while applying its block
rules to an ephemeral host solely for minimal-finalization evidence, using one
test allowance that is not a selected built-in profile.
...
- run a separate disposable-host `script/test-host-block-candidate` job whose
  single resident transient service applies standard block controls to that
  host with only a test allowance for GitHub's documented log/summary
  receiver, emits only `host_block_candidate_test_only` readiness/evidence,
  and leaves controls installed so the job result measures one minimal
  finalization candidate; and
...
6. Phase 4A first attempts strict no-profile host blocking; if that cannot
   produce a completed workflow result, it tests a source-documented
   log-receiver allowance in a separate disposable job. That candidate writes
   only `host_block_candidate_ready_no_public_activation`, treats its
   allowance as test policy rather than a selected profile, and treats job
   completion with controls still resident as evidence only.
...
- attempt strict no-profile host-network block finalization in an explicitly
  test-only disposable job, then test only source-documented candidate
  allowances if zero implicit egress does not complete;
...
A separate host candidate may emit `host_block_candidate_test_only` and
`host_block_candidate_ready_no_public_activation` only on an ephemeral host
after standard block networking and host lockdown verify with its narrowly
documented log-receiver test allowance. Completion of that candidate job is
evidence only for the profile decision; it cannot activate public `run`,
establish an Action interface, or prove compatibility for other egress.
