...
| `check-support` | Inspect required host capabilities and emit supported/unsupported reasons. | None |
| `render-plan --config` | Validate JSON, resolve hostnames under bounds, and render the exact frozen plan/report preview. | None |
| `run --config` | Execute the resident selected-mode lifecycle from root-owned input. | Yes |
...
| `invocation_id` | Required bounded non-secret identifier used for owned names and derived state paths. |
| `platform_profile` | Optional only after a default profile is proved; accepts a pinned identifier or `"none"`. |
...
Per-query timeouts must be bounded inside the total DNS budget. If limits are
exceeded, Fence fails before mutation; it does not truncate effective policy.
Findings may be capped after readiness, provided reports expose truncation and
...
6. Compute the effective-policy hash.
...
The current Rust CLI is scaffolding only. It proves this maintenance surface;
it does not implement the agent defined above.
...
No privileged host mutation is introduced in this phase.
