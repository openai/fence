...
After protected-runtime checks and bounded file reads, rejected resident or DNS evidence may emit one `Fence failure diagnostic (unverified)` line before rethrowing the original error. This is separate from `FENCE_REPORT_JSON` and makes no protection claim. Allow only fixed source/status labels, safe numeric health fields, and critical counts/codes within the shared report limits. Codes must come from the closed diagnostic allowlist; unknown codes use a fixed placeholder. Include omission/truncation indicators and exclude raw messages and host details. Diagnostic failures must preserve the original error; freshness checks and enforcement remain unchanged.
