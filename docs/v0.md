...
of these paths selects a platform profile. A separate strict-none candidate
applies the composed block lifecycle on one disposable host with no user or
implicit allowances and reports only test-only readiness; successful job
completion can prove that minimal finalization path, not general compatibility
or public activation. This evidence is not reachable from the public CLI. A distinct packaged-artifact acceptance
...
Phase 4A may additionally execute a dedicated strict-none host candidate on a
separate ephemeral runner. That candidate accepts only standard block mode,
`platform_profile: "none"`, and an empty user allowance list; it applies the
composed controls on the host and may write only
...
minimal finalization path with controls left installed until teardown. It is
not a public protection assertion, an Action interface, or proof that
workflows requiring new network activity remain compatible.
...
protection available to public execution. A distinct strict-none host
...
rules to an ephemeral host solely for minimal-finalization evidence.
...
- run a separate disposable-host `script/test-host-block-candidate` job whose
  single resident transient service accepts only strict no-profile empty
  policy, applies standard block controls to that host, emits only
  `host_block_candidate_test_only` readiness/evidence, and leaves controls
  installed so the job result measures one minimal finalization path; and
...
6. Phase 4A attempts a strict-none host-block candidate in a separate
   disposable job. It accepts no platform profile or user allowance, writes
   only `host_block_candidate_ready_no_public_activation`, and treats job
   completion with controls still resident as evidence for the minimal
   finalization path only.
...
- attempt strict no-profile host-network block finalization in an explicitly
  test-only disposable job before defining any implicit platform allowance;
...
A separate strict-none candidate may emit `host_block_candidate_test_only`
and `host_block_candidate_ready_no_public_activation` only on an ephemeral
host after standard block networking and host lockdown verify with no
allowances. Completion of that candidate job is evidence only for its minimal
finalization path; it cannot activate public `run`, establish an Action
interface, or prove compatibility for steps requiring new egress.
