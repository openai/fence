...
On a reviewed GitHub-hosted Ubuntu LTS x64 runner, public Linux `run`
...
```text
GitHub-hosted Ubuntu LTS x64 runner
...
| Image label | `ubuntu-24.04` or `ubuntu-latest`, resolving to a reviewed Ubuntu LTS release |
| Architecture | `x86_64` |
...
Both runner labels are release-validated. Fence accepts GitHub-hosted x64 runners on reviewed even-year Ubuntu LTS releases from 24.04 through 40.04 and verifies the security properties required by the selected mode. Before trusted executable capture, the root resident may restore root ownership only to the exact reviewed `/etc` and `/usr` directories when their existing ownership and access match the known hosted-image shape.

After this limited ownership repair, Fence must inspect required capabilities before any other security-state mutation.
...
- native platform targets other than reviewed Ubuntu LTS x64;
...
Successful `check-support` data additionally contains `hosted_runner_fingerprint` with schema version `3`, a reviewed hosted-runner reference, the observation method, and status `accepted_reference_not_checked`. The embedded fingerprint is a reviewed reference, not a live-host equality requirement. The read-only probe does not execute activation-time capability checks, and its response is not evidence that protection is active.
...
The Action checks native inputs and advanced raw JSON against the same configuration contract before installing privileged runtime state. It rejects duplicate JSON object fields, noncanonical integer tokens, unknown or missing fields, invalid destinations, unsupported modes or profiles, and incompatible container or artifact policies. Native allowlists are limited to 64 unique normalized entries; advanced JSON is limited to 64 physically declared entries. Valid advanced JSON reaches the agent without reserialization.
...
5. In protected `run`, prehydrate every required exact platform and user hostname in bounded parallel batches of at most 32 before firewall activation, materialize every configured transport, and record the active address realization separately from the logical hash. Each required hostname retries only transient or addressless `A`/`AAAA` rounds, at most three attempts within one shared ten-second startup deadline. Integrity-invalid responses immediately cancel sibling bootstrap work, and post-ready refresh remains single-round. User wildcard patterns never prehydrate or delay readiness; matching concrete names materialize on demand. After required roots complete, the optional hosted-runner watchdog receives one bounded round within the remaining shared deadline; a transient empty answer does not prevent readiness.

After readiness, platform roots refresh every five seconds. Exact user roots refresh from bounded observed TTLs, no later than sixty seconds and before their active authorization expires. Due roots are refreshed in deadline order with deterministic tie-breaking, and their next deadlines are measured from completed refreshes. Losing any required exact-root coverage is a critical failure even if another root refreshed successfully. A wildcard-matched name remains admitted for the invocation lifetime while its materialized rules retain only the bounded observed TTL plus refresh overlap. CNAME descendants inherit the complete transport set of their exact or admitted wildcard source hostname. Address rotation changes the active `ruleset_hash` without changing the logical `policy_hash`.
...
The selected `github_hosted_workflow_bootstrap_v5` descriptor is a narrow exception: its trusted-launcher realization bind-mounts a deterministic root-owned resolver file read-only over the reviewed hosted-runner resolver target. Host processes therefore query the root-resident mediator directly, Docker keeps a separate local route, and upstream UDP DNS is permitted only from the mediator. Each existing host and Docker TCP DNS listener permits at most four concurrent clients and 16 frames per client, applies one monotonic two-second deadline to each incoming frame, and keeps malformed or slow connections from blocking other clients. Fatal security errors still fail the same supervised resident listener. Local TCP DNS queries are translated to the same bounded UDP upstream path; the protected firewall never grants upstream TCP DNS access.
...
Authorized results-storage hostnames, exact pinned-runner or `opt_in_github_artifact_dns` origin, counts, attribution failures, rejections, truncation, and the selected `allow_github_artifacts` setting are recorded in bounded DNS evidence. Dynamic
...
### Hosted runner security checks

Fence supports GitHub-hosted Ubuntu LTS x64 runners selected through `ubuntu-24.04` or `ubuntu-latest`. Approved versions are even-year `.04` releases from 24.04 through 40.04. Fence checks that runner observations are complete, bounded, and identify a supported runner. Its Rust agent then verifies trusted executable paths, effective runner access, sudo policy, container controls, and local services before reporting readiness. Routine image updates do not bypass these checks. The Azure platform agent is optional diagnostic information; its presence is not required for activation.
...
- an aggregate count of existing Docker workloads;
- reviewed names or domain-separated name hashes, complete source SHA-256 digests, and reduced direct-principal or runner-group `NOPASSWD` marker classifications of bounded root-owned sudo policy sources;
...
The serialized host identity is limited to a reviewed Ubuntu LTS release,
...
The local-control candidate tries at most three times to obtain two matching security-relevant private snapshots fifty milliseconds apart. Matching acquisition failures use the remaining attempts; they cannot establish a successful observation. Malformed rows, exceeded bounds, and incomplete ownership alone do not gain retries. Excluded inaccessible Unix listener identities and their evidence-only count do not affect stability; acquisition failures, bounds, unresolved listeners, retained root container processes, reachable Unix listeners, root TCP listeners, and their private owner identities still must match. It records explicit `stable`, `unstable`, `bounds_exceeded`, or `unavailable` status rather than silently accepting a partial scan. Failure diagnostics contain only that status, the attempt count, and fixed acquisition/bound reason codes. The Action validates those fields before including them in ordinary setup-failure logs.
...
Protected activation requires a stable, bounded observation with complete reachability and ownership. Runner groups must remain within the reviewed group set, and root container processes, reachable root Unix listeners, and root TCP listeners must be subsets of reviewed endpoint and owner identities. Fence captures the actual post-setup inventory as its resident baseline. Audit and block modes pin complete root-owned sudo policy sources and verify effective runner sudo behavior. Both block modes additionally prove no runner sudo grants remain; standard block also proves container access is unavailable. Every mode rechecks its captured baseline before readiness and every five seconds afterward. Incomplete evidence, unsupported privilege paths, added unreviewed endpoints, or later baseline drift fails closed.

The mutation probe is limited to `/`, `/etc`, `/etc/sudoers.d`, `/usr`, `/usr/bin`, and `/usr/sbin`; it uses an exclusive empty file, immediately deletes it as the runner when possible, and requires root cleanup after a partial result. It does not modify commands or sudo policy. The observer does not emit sudo policy text, environment values, process arguments, workload identifiers, arbitrary host files, credentials, or payload data. Observation output remains `observation_only_no_protection`. `check-support` remains read-only and does not claim that protection is active. The candidate classifier validates complete bounded schema-`4` evidence and the supported runner identity before always activating the Rust resident, which performs security-critical host verification. Malformed, internally inconsistent, incomplete, truncated, or oversized evidence fails closed.

Fingerprint schema `3`, observer schema `4`, bundle-manifest schema `4`, runtime-evidence schema `5`, and the Action input contract remain unchanged. Root-owned sudo policy files are captured from the live runner, pinned by their complete bytes and identities, and rechecked so later changes remain detectable.

The accepted reference records:

| Surface | Accepted observed shape |
| --- | --- |
| Host identity | Reviewed Ubuntu LTS from `24.04` through `40.04`, `x86_64`, principal `runner` |
| Required group consequence | Runner groups remain a subset of reviewed groups; Docker access is verified or proved absent before lockdown |
| Trusted executables | Reviewed root-owned regular files with pinned identities, canonical paths, safe modes, and secure ancestors; Docker is optional only when its services, sockets, and local-control paths cannot provide runner access |
...
| Passwordless sudo classification | Both block modes remove discovered runner sudo grants and reject any remaining runner sudo access |
| Sudo policy integrity | Every bounded root-owned sudo source is pinned by its complete contents and identity; block mode proves runner sudo access is removed |
| Container activation paths | Standard block stops and runtime-masks available Docker and containerd paths, or proves the runtimes are already unavailable |
...
| Local root-control inventory | Bounded root processes and TCP/Unix listeners are a subset of reviewed endpoint and owner identities; the live post-setup inventory becomes the resident baseline |

The trusted-launcher lifecycle verifies reviewed runner capabilities and rejects unsafe privilege changes. Destructive hosted integration proves the selected mode remains secure across compatible runner image updates.
...
- validate the supported runner, trusted executable paths, and bounded root-owned sudo policy before mutation, ignoring backup files that sudo ignores and accepting safe `Defaults` alongside a reviewed runner grant;
- discover each reviewed runner sudo grant, capture its source bytes, mode, owner, device, inode, and digest only in memory, and compare the source again immediately before removal;
- fail if any runner sudo grant remains after removing the reviewed sources;
- fail before ready if lockdown cannot be established;
- verify through pinned nested sudo and a root-issued runner policy listing that no runner sudo command remains available; and
- never restore broad sudo access after readiness.

If setup fails before readiness, rollback recreates captured sources only
...
For protected block mode with `container_policy: "disable"`, Fence must remove ordinary Docker/containerd control paths that would let later workflow code regain host-equivalent authority. It rejects existing workloads, stops and runtime-masks available reviewed Docker/containerd activation paths, verifies daemon and socket access is unavailable, and allows only reviewed root-service identities. A runner without Docker access is accepted only after Fence verifies its services, sockets, and local-control paths cannot provide container access, whether or not the Docker executable is installed. The actual post-lockdown inventory becomes the resident baseline; added endpoints, owner changes, unstable scans, or incomplete scans are critical. Tests must cover:

- present runtime services and their activation paths, or verified runtime absence;
...
The post hook must validate the five fixed DNS rejection counters and their total before including them in `warnings.materialization_rejection_reasons`. Counts must be nonnegative safe JSON integers and sum exactly to `warnings.materialization_rejections`. Unknown reason keys, malformed counts, inconsistent totals, or a firewall-update rejection without its critical finding must fail evidence validation. A validated critical report whose 64-finding list is truncated may omit that particular code, but must still be emitted as critical before the job fails. Human warnings use fixed reason labels rather than raw DNS data or error messages.
...
Rejected resident or DNS report validation may emit a single `Fence failure diagnostic (unverified)` line before rethrowing the original error. This diagnostic is separate from `FENCE_REPORT_JSON` and cannot establish verified protection. It projects only a fixed evidence-source label, recognized resident status, safe-integer verification sequence and heartbeat age, a bounded critical-finding count, at most five codes from a closed source-reviewed vocabulary, and omission/truncation indicators. Unknown codes use a fixed placeholder; raw messages, host details, and arbitrary report fields are never emitted. Existing protected-runtime checks and bounded file reads precede this projection. Diagnostic generation or logging failure must not replace the validation error, relax freshness checks, retry validation, or restore controls.
...
- Action acceptance runs each unique production-shaped end-to-end case on a fresh runner while retaining coverage of standard block, degraded block, audit observation, failure propagation, and repeated quiet finalization; source-candidate artifacts are retained for 30 days so failed-job reruns can reuse the original verified bundle;
...
- the non-quiet zero-input acceptance job verifies the complete denied direct-IP packet burst and its counters within a bounded wait, returning early when complete and never resending the burst; timeout diagnostics contain only bounded counters and a short list of missing fixed test destinations. The read-only downstream verifier proves that its single post-hook structured report retains the final blocked UDP probe destinations while explicitly disclosing omitted rows; the quiet finalization replicas remain synthetic-traffic-free, require healthy verified controls, and may report only fully classified noncritical DNS refusals without other warnings or omissions;
...
A separate non-required `nightly` workflow runs daily, by input-free manual dispatch, and for same-repository pull requests targeting `main`. Scheduled and manual runs use the exact `main` commit; pull requests use GitHub's exact same-repository merge commit. The workflow builds an ephemeral production-shaped candidate on fixed `ubuntu-24.04` and reuses the complete unique-case Action-acceptance suite on `ubuntu-latest`. It is not a branch-protection or release gate; only reviewed Ubuntu LTS versions and security-relevant capabilities are accepted.

A separate non-required drift canary runs every six hours, supports manual dispatch with an optional full Action SHA, and requires an explicit full SHA when invoked as a reusable release-validation workflow. Every invocation tests zero-input standard block and explicit audit mode on both `ubuntu-24.04` and `ubuntu-latest`; no runner label or mode may skip activation.
