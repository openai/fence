...
Fence is not an enforcement agent today. The current Phase 3B repository
...
paths now execute native network-policy, bounded NFLOG findings, and a
test-only resident transient-service lifecycle inside disposable namespaces.
They may write readiness labelled `test_only_ready_no_protection` beneath an
injected scratch root, never production `/run/fence` readiness. This evidence
is not reachable from the public CLI. A distinct packaged-artifact acceptance
...
The current Phase 3B binary emits versioned JSON only. Successful command results are written to
...
and atomic bounded report replacement below an injected scratch root. The
transient test service may write a `test_only_ready_no_protection` document
only after its namespace-local network policy is applied and verified; it does
not write `/run/fence`, disable sudo, disable containers, or make public
`run` executable.
...
The current Phase 3B Rust CLI implements strict JSON parsing, deterministic
...
apply, read, verify, rollback, bounded NFLOG conversion, and transient-service
resident verification operations; they write evidence status
`network_enforcement_test_only` or `resident_lifecycle_test_only`, with only
the latter allowed to write explicitly non-protecting test readiness below an
injected scratch root.
...
Later Phase 3 and Phase 4 privileged tests add sudo/container lockdown and
measurement of any proposed platform finalization profile. The observation,
network, and resident-service tests do not by themselves establish protection.
...
privileged backend, NFLOG socket, secure runtime-file, and resident-service
modules. That exception is bounded to code whose meaningful failure behavior
depends on Linux root capabilities, kernel state, netlink sockets, or
transient system services; ordinary unit tests still cover deterministic
normalization, packet-prefix reduction, path-safety, readiness ordering, and
resident scheduling helpers, and the privileged workflow is the required
behavioral proof.
...
During Phase 3B, the last three lifecycle assertions apply only to
`resident_lifecycle_test_only` evidence beneath an injected test root. A
test-only ready file is never a protection assertion, and public `run`
